...
两个小鹤主方案的 `speller/max_code_length` 调整为 `20`，避免输入完整英文时在 5 位处被前缀候选提前上屏。例如手动输入 `english` 时，不会在 `englis` 阶段先提交 `english` 再留下 `sh`。

当前折中是：只在连续英文达到 4 位后再触发英文候选，例如 `main -> main`、`engli -> english`，并把英文候选质量设为 `9000`，让中文主词库优先，英文候选作为补充进入候选页。英文和中文编码不可能完全不碰撞，但较长英文才触发，日常干扰会小很多。
